import lib.AWSIoTPythonSDK.MQTTShadowManager as shadowManager
import lib.AWSIoTPythonSDK.MQTTDeviceShadow as deviceShadow

from lib.AWSIoTPythonSDK import MQTTConst
from lib.AWSIoTPythonSDK.MQTTClient import MQTTClient


class AWSIoTMQTTClient:

    def __init__(self, clientID, protocolType=MQTTConst.MQTTv3_1_1, useWebsocket=False, cleanSession=True):
        self._mqttClient = MQTTClient(clientID, cleanSession, protocolType)
    ...
    def configureCredentials(self, CAFilePath, KeyPath="",
                             CertificatePath=""):  # Should be good for MutualAuth certs config and Websocket rootCA config
        self._mqttClient.configCredentials(CAFilePath, KeyPath, CertificatePath)

    def configureAutoReconnectBackoffTime(self, baseReconnectQuietTimeSecond, maxReconnectQuietTimeSecond,
                                          stableConnectionTimeSecond):
        self._mqttClient.setBackoffTiming(baseReconnectQuietTimeSecond, maxReconnectQuietTimeSecond,
                                          stableConnectionTimeSecond)
...
class AWSIoTMQTTShadowClient:
    ...
    def __init__(self, clientID, protocolType=MQTTConst.MQTTv3_1_1, useWebsocket=False, cleanSession=True):
        ...
        self._AWSIoTMQTTClient.configureOfflinePublishQueueing(
            0)  # Disable queueing, no queueing for time-sentive shadow messages
    ...
    def configureCredentials(self, CAFilePath, KeyPath="",
                             CertificatePath=""):  # Should be good for MutualAuth and Websocket
        self._AWSIoTMQTTClient.configureCredentials(CAFilePath, KeyPath, CertificatePath)

    def configureAutoReconnectBackoffTime(self, baseReconnectQuietTimeSecond, maxReconnectQuietTimeSecond,
                                          stableConnectionTimeSecond):
        self._AWSIoTMQTTClient.configureAutoReconnectBackoffTime(baseReconnectQuietTimeSecond,
