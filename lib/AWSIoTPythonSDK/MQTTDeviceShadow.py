...
class deviceShadow:
            ...
            if self._basicJSONParserHandler.validateJSON():  # Filter out invalid JSON
                ...
                if currentToken is not None and currentToken in self._tokenPool.keys():  # Filter out JSON without the desired token
                    ...
                    if self._shadowSubscribeCallbackTable.get(currentAction) is not None:
                        self._shadowManagerHandler.insertShadowCallback(
                            self._shadowSubscribeCallbackTable[currentAction], payloadUTF8String, currentType,
                            currentToken)
    ...
    def shadowGet(self, srcCallback, srcTimeout):
        ...
        # Start the timer
        self._tokenPool[currentToken] = Timer.Alarm(self._timerHandler, srcTimeout, arg=("get", currentToken),
                                                    periodic=False)
        return currentToken
    ...
    def shadowDelete(self, srcCallback, srcTimeout):
        ...
        # Start the timer
        self._tokenPool[currentToken] = Timer.Alarm(self._timerHandler, srcTimeout, arg=("delete", currentToken),
                                                    periodic=False)
        return currentToken
    ...
    def shadowUpdate(self, srcJSONPayload, srcCallback, srcTimeout):
        ...
        if self._basicJSONParserHandler.validateJSON():
            ...
            # Start the timer
            self._tokenPool[currentToken] = Timer.Alarm(self._timerHandler, srcTimeout, arg=("update", currentToken),
                                                        periodic=False)
        else:
